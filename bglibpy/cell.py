...
class Cell(object):
    ...
    def potential_pointsource_current(self, current, position, target, sigma):
        """ Calculates the potential at the location specified by 'position' due to a point source current of amplitude 'current' (in nA) at the location specified by 'target'
        Assumes an infinite homogeneous medium with conductivity specified by 'sigma' (in nA/um) ."""

        distance = ((position[0]-target[0])**2+(position[1]-target[1])**2+(position[2]-target[2])**2)**0.5

        try:
            return current/(4*numpy.pi*sigma*distance) * 1e3 # converts to mV
        except:
            return current[:,numpy.newaxis]/(4*numpy.pi*sigma*distance) * 1e3 # converts to mV

    def activation_pointsource_current(self, current, position, target, sigma):
        """ Calculates the second spatial derivative of the potential field over the segment with start and end points specified by 'position' due to a point source current of amplitude 'current' (in nA) at the location specified by 'target'
        Assumes an infinite homogeneous medium with conductivity specified by 'sigma' (in nA/um) ."""

        xlist = numpy.linspace(position[0][0],position[1][0],3)
        ylist = numpy.linspace(position[0][1],position[1][1],3)
        zlist = numpy.linspace(position[0][2],position[1][2],3)

        potentials = []

        for i in range(len(xlist)):

            potentials.append( self.potential_pointsource_current(current, numpy.array([ xlist[i], ylist[i], zlist[i] ]), target, sigma ) )

        potentials = numpy.array(potentials)

        dist = numpy.sqrt( (xlist[0]-xlist[1])**2 + (ylist[0]-ylist[1])**2 + (zlist[0]-zlist[1])**2 )

        return (potentials[0]-2*potentials[1]+potentials[2])/dist**2 # converts to mV/um**2
        #return current[:,numpy.newaxis]/(4*numpy.pi*sigma*distance) * 1e3 # converts to mV

    def add_ICMS(self,loc,amp,sigma,startTime,endTime,duration):

        positions = {}

        """Adds an extracellular potential generated by a point source current of amplitude 'amp' at position 'loc'. Requires specification of conductivity (must be homogeneous infinte medium)
        and of the start time and end time of the stimulus, and duration of the simulation. """
        all_sections = self.cell.getCell().all

        tvec = numpy.linspace(0,duration,duration*100)
        Tvec = neuron.h.Vector().from_python(tvec)
        self.persistent.append(Tvec)

        vVec = numpy.zeros_like(tvec)
        vVec[numpy.intersect1d(numpy.where(tvec > startTime), numpy.where(tvec < endTime))] = 1
        ...
        for section in all_sections:
            ...
            for seg in section:
                ...
                position = self.get_positions(section,seg.x)

                name = section.name()+"(" + str(seg.x)+")"

                positions[name] = position

                potential = self.potential_pointsource_current(amp,position,loc,sigma)
                potentials.append(potential)

                if len(prevPos) > 0:

                    activation = self.activation_pointsource_current(amp,[prevPos, position],loc,sigma)
                    activations.append(activation)

                prevPos = position


                inputVec = neuron.h.Vector().from_python(vVec*potential)
                self.persistent.append(inputVec)
                self.persistent.append(seg.extracellular)
                self.persistent.append(seg.extracellular.e)
                out = inputVec.play(seg.extracellular._ref_e,Tvec,True)
                self.persistent.append(out)
                #self.persistent.append(seg._ref_vext[0])
                #self.persistent.append(seg.vext[0])
                #self.persistent.append(seg.extracellular)


        return potentials, activations, positions


    def add_ICMS_noAx(self,loc,amp,sigma,startTime,endTime,duration):

        """Adds an extracellular potential generated by a point source current of amplitude 'amp' at position 'loc'. Requires specification of conductivity (must be homogeneous infinte medium)
        and of the start time and end time of the stimulus, and duration of the simulation. NOTE: THIS FUNCTION IS BROKEN"""
        all_sections = self.cell.getCell().all
        ...
        for section in all_sections:
            if "soma" in section.name() or "apic" in section.name() or "dend" in section.name():
                section.insert('extracellular')
        ...
        numpy.save('e.npy',seg.extracellular.e)

    def add_constant_ICMS(self,loc,amp,sigma,truncated=False):
        """Adds a constant extracellular potential generated by a point source of amplitude 'amp' at location 'loc', in an infinte homogeneous medium of sonductivity 'sigma'."""
        all_sections = self.cell.getCell().all
        ...
        for section in all_sections:
            ...
            for seg in section:
                ...
                if truncated==True and 'axon' in section.name():
                ...
                seg.extracellular.e = potential
                #seg.vext[0] = potential
                self.persistent.append(seg.extracellular)
                #self.persistent.append(seg.vext[0])
                numpy.save('e.npy',seg.extracellular.e)


    def add_allsegments_voltagerecordings(self):
        """Add a voltage recording to every section of the cell."""
        all_sections = self.cell.getCell().all
        listnames = []

        for section in all_sections:
            eval('neuron.h.'+section.name()+'.insert(\'extracellular\')')
            for segment in section:
                var_name = 'neuron.h.' + section.name() + "(" + str(segment.x)+")._ref_v"
                listnames.append(var_name)

        self.add_recordings(listnames)

    def get_allsegments_voltagerecordings(self):
        """Get all the current recordings from all the segments.
        ...
        for section in all_sections:
            for segment in section:
                var_name = 'neuron.h.' + section.name()+"(" + str(segment.x)+")._ref_v"
                allSectionCurrents[section.name()+"(" + str(segment.x)+")"] = self.get_recording(var_name)

        return allSectionCurrents

    def add_allsegments_currentrecordings(self):
        """Add a current recording to every segment of the cell."""
        all_sections = self.cell.getCell().all
        listnames = []
        ...
        for section in all_sections:
            # eval('neuron.h.'+section.name()+'.insert(\'extracellular\')')
            for segment in section:
                var_name = 'neuron.h.' + section.name() + "(" + str(segment.x)+")._ref_i_membrane_"
                listnames.append(var_name)


        self.add_recordings(listnames)

    def add_allsegments_sodiumrecordings(self):
        """Add a current recording to every segment of the cell."""
        all_sections = self.cell.getCell().all
        listnames = []
        ...
        for section in all_sections:
            # eval('neuron.h.'+section.name()+'.insert(\'extracellular\')')
            for segment in section:
                baseName = 'neuron.h.' + section.name() + "(" + str(segment.x)+")._ref_"
                names = ["gNaTg_NaTg","gNa12_Na12Hu2009","gNa16_Na16Hu2009","gNap_Et2_Nap_Et2","gNaTa_t_NaTa_t","gNaTs2_t_NaTs2_t"]
                for name in names:
                    var_name = baseName + name
                    #if 'apic' in section.name() or 'soma' in section.name():
                    try:
                        self.add_recordings([var_name])
                    except:
                        pass


    def get_allsegments_currentrecordings(self,clamp = True,duration = 2000,pulse_start_time = 500, pulse_end_time = 2000,pulse_amp = 0.5):
        """Get all the current recordings from all the segments.
        ...
        for section in all_sections:
            for segment in section:
                var_name = 'neuron.h.' + section.name()+"(" + str(segment.x)+")._ref_i_membrane_"
                #area = neuron.h.area(segment.x,section.name())*1e-8 #  square microns to cm^2
                area = 1
                allSectionCurrents[section.name()+"(" + str(segment.x)+")"] = self.get_recording(var_name)*area #mA to nA

        if clamp:
            somaName = list(allSectionCurrents)[0]

            times = numpy.linspace(0,duration,len(allSectionCurrents[somaName]))

            pulseIdx = numpy.intersect1d(numpy.where(times>pulse_start_time),numpy.where(times<pulse_end_time))

            allSectionCurrents[somaName][pulseIdx] -= pulse_amp

        return allSectionCurrents

    def get_allsegments_sodiumrecordings(self):
        """Get all the current recordings from all the segments.
        ...
        for section in all_sections:
            for segment in section:
                ...
                area = 1
                #if 'apic' in section.name() or 'soma' in section.name():
                allSectionCurrents[section.name()+"(" + str(segment.x)+")"] = 0
                for name in names:
                    var_name = baseName+name
                    try:
                        allSectionCurrents[section.name()+"(" + str(segment.x)+")"] += self.get_recording(var_name)*area #mA to nA
    ...
    def add_LFP(self):
        """ Adds curretn recording to all section for use with LFP calculation"""

        self.add_allsegments_currentrecordings()

    def get_close_LFP(self,loc,sigma,truncated = False,noMod=False,clamp = False,duration = 2000,pulse_start_time = 500, pulse_end_time = 2000,pulse_amp = 0.5):


        all_sections = self.cell.getCell().all
        allSectionCurrents = self.get_allsegments_currentrecordings(clamp,duration,pulse_start_time,pulse_end_time,pulse_amp)
        ...
        for section in all_sections:
            ...
            if noMod == True:
                n = "axon"
            if n in section.name():
                for segment in section:
                    position = self.get_positions(section,segment.x)
                    lastPosition = position
                    break
                break
        ...
        for section in all_sections:
            ...
            if "soma" in section.name():
                position = self.get_positions(section,0.5)

                current = allSectionCurrents[section.name()+"(0.5)"]

                potential += self.potential_pointsource_current(current,position,loc,sigma)

                potentials.append(potential)

                #lastPosition = self.get_positions(section,1.0)
                ...
                parent = neuron.h.SectionRef(sec=section).parent

                print("Parent is")
                print(parent)
                print(parentSeg.x)

                positions.append(self.get_positions(parent,parentSeg.x))


                segLength = section.L/section.nseg

                for segment in section:

                    if truncated==True and ('axon' in section.name() or 'ais' in section.name()):
                    ...
                    currents.append( allSectionCurrents[section.name()+"(" + str(segment.x)+")"] )

                for i, current in enumerate(currents):

                    if numpy.array(loc).ndim>1:


                        x1 = loc[0]-positions[i+1][0]
                        y1 = loc[1]-positions[i+1][1]
                        z1 = loc[2]-positions[i+1][2]

                        xdiff = positions[i+1][0]-positions[i][0]
                        ydiff = positions[i+1][1]-positions[i][1]
                        zdiff = positions[i+1][2]-positions[i][2]

                        h = 1/segLength * (x1*xdiff + y1*ydiff + z1*zdiff)
                    else:
                        h = 1/segLength * numpy.matmul((loc-positions[i+1]),(loc-positions[i]))


                    r2 = (loc[0]-positions[i][0])**2 + (loc[1]-positions[i][1])**2 + (loc[2]-positions[i][2])**2 - h**2
                    l = h + segLength

                    segPotential = current/(4*numpy.pi*sigma*segLength)*numpy.log(numpy.abs(((h**2+r2)**.5-h)/((l**2+r2)**.5-l)))

                    segPotential *= 1e3 #converts to mV

                    potentials.append(segPotential)

                    potential += segPotential


        return potential, potentials


    def get_close_LFP_noAx(self,loc,sigma,truncated = False,clamp = False,duration = 2000,pulse_start_time = 500, pulse_end_time = 2000,pulse_amp = 0.5):
        ...
        for section in all_sections:
            ...
            if "soma" in section.name():
                ...
                potentials.append(self.potential_pointsource_current(current,position,loc,sigma))

            elif "soma" in section.name() or "apic" in section.name() or "dend" in section.name():
                ...
                lastPosition = self.get_positions(parent,parentSeg.x)
                ...
                for i, current in enumerate(currents):
                    ...
                    segPotential = current/(4*numpy.pi*sigma*segLength)*numpy.log(numpy.abs(((h**2+r2)**.5-h)/((l**2+r2)**.5-l))) * 1e3
