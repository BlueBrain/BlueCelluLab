...
def tanhtanh(x1, x2):
    return (1. + np.exp(-2. * (x1 + x2)) - np.exp(-2. * x1) - np.exp(-2. * x2)) / \
        (1. + np.exp(-2. * (x1 + x2)) + np.exp(-2. * x1) + np.exp(-2. * x2))
# one minus tanhtanh, for stability


def one_minus_tanhtanh(x1, x2):
    return 2. * (np.exp(-2 * x1) + np.exp(-2 * x2)) / \
        (1. + np.exp(-2. * (x1 + x2)) + np.exp(-2. * x1) + np.exp(-2. * x2))
...
# HH kinetic rates
def alphan(V):
    return 0.01 * vtrap(-(V + 55.0), 10.0) * 1.0e3


def betan(V):
    return 0.125 * np.exp(-(V + 65.0) / 80.0) * 1.0e3


def alpham(V):
    return 0.1 * vtrap(-(V + 40.0), 10.0) * 1.0e3


def betam(V):
    return 4.0 * np.exp(-(V + 65.0) / 18.0) * 1.0e3


def alphah(V):
    return 0.07 * np.exp(-(V + 65.0) / 20.0) * 1.0e3


def betah(V):
    return 1.0 / (np.exp(-(V + 35.0) / 10.0) + 1.0) * 1.0e3

# derivatives of HH kinetic rates


def alphandot(V):
    return -0.01 * (np.exp(-(V + 55.) / 10.) - 1. + (V + 55.) *
                    np.exp(-(V + 55.) / 10.) / 10.) / np.power(np.exp(-(V + 55.) / 10.) - 1., 2) * 1.0e3


def betandot(V):
    return -0.125 * np.exp(-(V + 65.) / 80.) / 80. * 1.0e3


def alphamdot(V):
    return -0.1 * (np.exp(-(V + 40.) / 10.) - 1. + (V + 40.) *
                   np.exp(-(V + 40.) / 10.) / 10.) / np.power(np.exp(-(V + 40.) / 10.) - 1., 2) * 1.0e3


def betamdot(V):
    return -4. / 10. * np.exp(-(V + 65.) / 10.) * 1.0e3


def alphahdot(V):
    return -0.07 / 20. * np.exp(-(V + 65.) / 20.) * 1.0e3


def betahdot(V):
    return 0.1 * np.exp(-(V + 35.) / 10.) / \
        np.power(np.exp(-(V + 35.) / 10.) + 1., 2) * 1.0e3


class greensFunctionCalculator:
    def __init__(self,
                 dend_diams=np.array([0.45,
                                      1.]),
                 dend_lengths=np.array([937.5,
                                        559.016994375]),
                 somaL=25.,
                 somaDiam=25.,
                 C_m=0.8,
        ...
        if HH == 1:
            ...
            alphain = np.array([.1 * vtrap(-(E_eq + 40.), 10.), .07 *
                                np.exp(-(E_eq + 65.) / 20.), .01 * vtrap(-(E_eq + 55.), 10.)])
            betain = np.array([4. * np.exp(-(E_eq + 65.) / 18.), 1. / (
                np.exp(-(E_eq + 35.) / 10.) + 1.), .125 * np.exp(-(E_eq + 65.) / 80.)])
            sv = alphain / (alphain + betain)
            self.g_s = (self.gNa * sv[0] * sv[0] * sv[0] * sv[1] + self.gK *
                        sv[2] * sv[2] * sv[2] * sv[2] + self.gL) / somaA  # S/cm^2
            self.E_eq = E_eq
        else:
            self.E_eq = EL
    ...
    def calc_somaImpedance(self, s, E):
        if self.HH == 1:
            ...
            z_K2 = (s + alphan(E) + betan(E)) / (4. * self.gK * n * n * n *
                                                 (self.EK - E) * (alphandot(E) - n * (alphandot(E) + betandot(E))))
            z_Na1 = -1. / (self.gNa * m * m * m * h)
            z_Na2 = (s + alpham(E) + betam(E)) / (3 * self.gNa * m * m * h *
                                                  (self.ENa - E) * (alphamdot(E) - m * (alphamdot(E) + betamdot(E))))
            z_Na3 = (s + alphah(E) + betah(E)) / (self.gNa * m * m * m *
                                                  (self.ENa - E) * (alphahdot(E) - h * (alphahdot(E) + betahdot(E))))
            z_soma = 1. / (1. / z_C - 1. / z_K2 - 1. / z_L -
                           1. / z_K1 - 1. / z_Na1 - 1. / z_Na2 - 1. / z_Na3)
    ...
    def greensFunctionFrequency(
            self, synapsedendrite, synapseloc, samplefreqs):
        '''
        ...
        if nosynseg.size:
            for j in range(len(nosynseg)):
                ...
                z_m = 1. / (2. * math.pi *
                            self.diams[i] * (s * self.C_m + self.g_m))
                # space constant
                gamma = np.sqrt(z_a / z_m)
                # characteristic impedance
                z_c = z_a / gamma
                # modified soma impedance
                z_snew = 1. / ((1. / z_snew) + (1. / z_c) *
                               np.tanh(gamma * self.lengths[i]))
        ...
        G_synsoma = z_c * z_snew * np.cosh(gamma * D) * \
            one_minus_tanhtanh(gamma * D, gamma * L) / \
            (z_c + z_snew * tanh(gamma * L))
    ...
    def greensFunctionTime(self, synapsedendrite=0, synapseloc=0.5, N=np.power(
            2, 9), dt=0.1 * 1e-2, test=0, tmax=0.3):
        '''
        ...
        G_synsyn, G_synsoma, GI_synsoma, GI_awaysoma = self.greensFunctionFrequency(
            synapsedendrite, synapseloc, samplefreqs)
        ...
        return np.array(Gt_synsyn) / 1000., np.array(Gt_synsoma) / 1000., np.array(
            GIt_synsoma) / 1000., np.array(GIt_awaysoma) / 1000., t * 1000
    ...
    def calcFFT(self, arr, N, dt, test=0, method='hannig'):
        ...
        if method == 'standard':
            ...
            fftarr = ds / (6. * math.pi) * (
                -arr[0] * np.exp(-1j * smax * t) + 2. * N * np.exp(-1j * smax * t) *
                arreventrans + 4. * N * np.exp(-1j * (smax - ds) * t) * arroddtrans + arr[-1] * np.exp(1j * smax * t))
            ...
            for i in range(len(t)):
                fftarr[i] = integ.romberg(
                    self.aux_func, -smax, smax, args=[t[i]])
    ...
    def aux_func(self, s, t):
        return (self.greensFunctionFrequency(
            0, 0.95, np.array([s]))[0] * np.exp(1j * s * t))[0]
...
# ~
    ...
    def trapezoidalCorrection(self, times):
        ...
        alpha0 = -(1. - np.cos(times)) / np.power(times, 2) + 1j * \
            (times - np.cos(times)) / np.power(times, 2)
...
class pneuron:
    def __init__(self,
                 numsynapses,
                 greensFunctions_synsoma,
                 greensFunctions_synsyn,
                 tgreensFunction,
                 taus=[1.5,
                       1.5],
    ...
    def run_syn(self, tmax, dt, spiketimes):
        ...
        # interpolate at the desired sampling frequency (timestep of the
        # simulation)
        G_synsoma = []
        G_synsyn = []
        for j in range(self.numsynapses):
            G_synsoma.append(np.interp(tsim, self.tgreensFunction,
                                       self.greensFunctions_synsoma[j, :]))
            G_synsyn.append(np.interp(tsim, self.tgreensFunction,
                                      self.greensFunctions_synsyn[j, :]))
        ...
        for l in range(int(tmax / dt)):
            ...
            for j in range(self.numsynapses):
                # spike injector
                if k[j] < len(spiketimes[j]) and l * dt <= spiketimes[j][k[j]
                                                                         ] and spiketimes[j][k[j]] < (l + 1) * dt:
                    B[j] += self.weight[j]
                    k[j] += 1
    ...
    def run_IC(self, tmax, dt, stimdur, stimamp):
        ...
        I[N + 1:N + int(stimdur / dt) + 1] = stimamp * \
            np.ones(int(stimdur / dt))
...
class aneuron:
    ...
    def run_syn(self, tmax, dt, spiketimes):
        ...
        G_synsyn = []
        for j in range(self.numsynapses):
            GI_synsoma.append(np.interp(
                tsim, self.tgreensFunction, self.greensCurrents_synsoma[j, :]))
            G_synsyn.append(np.interp(tsim, self.tgreensFunction,
                                      self.greensFunctions_synsyn[j, :]))
        G_synsyn = np.array(G_synsyn)
        ...
        for l in range(int(tmax / dt)):
            ...
            for j in range(self.numsynapses):
                ...
                # advance the synaptic voltage traces
                V_syns[j,
                       i] = self.E_eq - (dt * np.sum(G_synsyn[j, 1:-1][::-1] *
                                         V_syns[j, i - N + 1:i - 1] *
                                         synwindow[j, i - N + 1:i - 1]) + dt *
                                         G_synsyn[j, -1] * V_syns[j, i - N] *
                                         synwindow[j, i - N] / 2.) / (1. - dt * G_synsyn[j, 0] * synwindow[j, i] / 2.)
                # advance soma inputs
                Isoma[j] = - (GI_synsoma[j, 0] * V_syns[j, i] * synwindow[j, i] /
                              2. + np.sum(GI_synsoma[j, 1:-1][::-1] * V_syns[j, i - N + 1:i - 1] *
                              synwindow[j, i - N + 1:i - 1]) + GI_synsoma[j, -1] * V_syns[j, i - N] * synwindow[j, i - N] / 2.)
            # advance soma voltage
            V_soma[i] = V_soma[i - 1] - dt * oneovertau * \
                (V_soma[i - 1] - self.E_eq) + \
                dt * oneoverC * dt * np.sum(Isoma)
    ...
    def run_IC(self, tmax, dt, stimdur, stimamp, HH=0.):
        ...
        for l in range(int(tmax / dt)):
            ...
            # advance soma voltage
            V_soma[i] = V_soma[i - 1] - dt * oneovertau * \
                (V_soma[i - 1] - self.E_eq) + dt * oneoverC * dt * Isoma
            I_dend[i] = dt * Isoma
...
def compute_system(
    ...
    for i in range(numsyn):
        Gt_synsyntemp, Gt_synsomatemp, GIt_synsomatemp, GIt_awaysomatemp, ttemp = q.greensFunctionTime(
            syndend[i], synloc[i], dt=dtGreen, N=N, tmax=0.3)
...
if __name__ == '__main__':
    ...
    V_soma, time = compute_system([[20, 20.5, 21., 21.5, 22, 23, 24], [50., 70]], 200, 0.025, numsyn=2, syndend=[
                                  0, 1], synloc=[0.5, 0.8], gbar=[0.0001, 0.0004], decay=[5., 3.], E_rev=[-10., -20.])
